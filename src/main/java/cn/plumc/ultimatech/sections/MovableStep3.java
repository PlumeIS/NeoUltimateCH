...
public class MovableStep3 extends MovableStep {
    public Vec3 velocity = new Vec3(0.0, 0.0, 0.1);

    public MovableStep3(ServerPlayer player, Game game) {
        super(player, SectionLocation.get(4, 6), game);
        setProcess(SectionCounter.toTicks(6.0));
    }
    ...
    public void tickRun(int tickTime) {
        ...
        vfx(content.origin);
        if (process.at(6.0)) position = new Vec3(0.0, 0.0, 0.0);
    }

    public void vfx(Vec3 origin) {
        for (int i = 0; i < 10; i++) {
            Vec3 pos = origin.add(transform.rotateVector(new Vec3(0.5, 0.5, 0.0))).add(transform.rotateVector(movement.scale(i)));
            if (i == 0 || i == 9) {
